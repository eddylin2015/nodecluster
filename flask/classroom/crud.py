...
from classroom import login_required_auth, get_model,  storage
...
def list():
    ...
    assets, next_page_token = get_model().list(cursor=token)
...
def list_mine():
    ...
    if token:
        token = token.encode('utf-8')
    assets, next_page_token = get_model().list_by_user(
